...
export const SocialCards = ({ cards, className = "" }: SocialCardsProps) => {
  ...
  return (
    <section className={`flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20 ${className}`}>
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-6 z-30">
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-blue-500 scale-[1.3] shadow-sm shadow-blue-500/50" : "bg-muted-foreground/30"}`} />
            ))}
          </div>
