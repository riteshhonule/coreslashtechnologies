...
const GlassCard = ({ children, className = "", hoverEffect = true }: { children: React.ReactNode, className?: string, hoverEffect?: boolean }) => (
  <motion.div
    whileHover={hoverEffect ? { y: -5, boxShadow: "0 20px 40px rgba(0,0,0,0.5)" } : {}}
    className={`glass-card border border-white/10 rounded-3xl p-6 relative overflow-hidden ${className}`}
  >
...
const AccordionItem = ({ title, children, isOpen, onClick }: { title: string, children: React.ReactNode, isOpen: boolean, onClick: () => void }) => (
  <div className="border border-white/10 rounded-2xl mb-4 overflow-hidden bg-white/5 backdrop-blur-sm transition-all hover:border-white/20">
    <button
      onClick={onClick}
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <div className="px-6 pb-6 text-white/40 leading-relaxed border-t border-white/10 pt-4">
            {children}
          </div>
        </motion.div>
      )}
    ...
    <div className="relative min-h-screen bg-dark-black overflow-hidden font-sans text-white pt-0">
      ...
      <section className="relative min-h-[40vh] md:min-h-[70vh] flex items-center justify-center px-4 pt-2 md:pt-10 bg-transparent z-20">
        ...
        >
          ...
          >
            ...
            <Link to="/portfolio" className="btn-pill btn-glass text-white px-8 py-4">
              View Portfolio
            </Link>
          </motion.div>
        </motion.div>
      ...
      <section className="relative z-10 py-16 px-4 container mx-auto">
        <div className="grid lg:grid-cols-2 gap-16 items-center mb-16">
          ...
          >
            <div className="absolute -inset-4 bg-primary-purple/20 rounded-[3rem] blur-2xl -z-10" />
            <div className="relative rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/10 bg-dark-black">
              <img
                src={teamCollaborationImg}
                alt="Team Collaboration"
                className="w-full h-96 object-cover transform hover:scale-105 transition-transform duration-700"
              />
              <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-dark-black to-transparent text-white backdrop-blur-[2px]">
                <h3 className="text-2xl font-bold mb-2">Empowering Growth</h3>
                <p className="text-white/40">Delivering excellence in every line of code.</p>
              </div>
            </div>
          </motion.div>
          ...
          {[
            ...
            >
              ...
              <div className="relative z-10 flex gap-5 items-start">
                <div className={`p-3 rounded-2xl ${item.bgIcon} flex-shrink-0 ${item.text} border border-white/5`}>
                  <item.icon className="w-6 h-6" />
                </div>
                <div>
                  <h4 className={`text-xl font-bold mb-2 text-white`}>{item.title}</h4>
                  <p className="text-white/40 leading-relaxed text-sm">{item.content}</p>
                </div>
              </div>
            </motion.div>
          ))}
      ...
      {/* Logos Strip */}
      <section className="py-12 overflow-hidden border-y border-white/5 bg-transparent">
        <div className="container mx-auto px-4 text-center">
          <p className="text-xs font-bold text-white/20 uppercase tracking-[0.2em] border border-white/5 px-4 py-2 w-fit mx-auto rounded-full bg-white/5">Trusted by industry leaders</p>
        </div>
      </section>
        ...
        <div className="flex flex-wrap justify-center gap-10">
          {[
            ...
            },
            {
              ...
              glow: "shadow-[0_0_30px_rgba(69,3,185,0.2)]"
            },
            {
            ...
            >
              {/* Glow Background */}
              <div className={`absolute inset-0 rounded-full blur-xl opacity-0 group-hover:opacity-100 transition duration-500 bg-white/5`} />

              {/* Main Card */}
              <div className={`relative px-8 py-4 rounded-full bg-white/5 backdrop-blur-xl border border-white/10 ${cert.glow} flex items-center gap-4 transition-all duration-500 hover:bg-white/10`}>
                {/* Icon */}
                <div className={`p-2 rounded-full bg-white/5 border border-white/10 ${cert.color}`}>
          ...
          <div className="container mx-auto relative z-10">
            ...
            <div className="grid md:grid-cols-3 gap-8 mt-12">
              {testimonials.map((testim: any, idx: number) => (
                <GlassCard key={idx} className="bg-white/5 border-white/10">
                  <div className="flex gap-1 mb-4">
                    {[1, 2, 3, 4, 5].map((s) => <StarIcon key={s} className="w-5 h-5 text-accent-cyan fill-accent-cyan" />)}
                  </div>
                  <p className="text-white/60 mb-6 italic text-sm">"{testim.text}"</p>
                  <div className="flex items-center gap-4">
                    <img src={testim.img} alt={testim.name} className="w-12 h-12 rounded-full object-cover ring-2 ring-white/10" />
                    <div>
                      <h4 className="font-bold text-white text-sm">{testim.name}</h4>
                      <p className="text-xs text-white/40">{testim.role}</p>
                    </div>
                  </div>
                </GlassCard>
              ))}
